...
1. Download the `mystery_gene.fasta` file from the Moodle and open it on `R`. This is a nucleotide strand from a segment of a chromosome of a cool species that we will try to identify.
...
12. Extract the FASTA protein sequence from the LOC106470356 gene (You can find it on the RefSeq submenu, on the mRNA and Protein segment, click on the NP_001301089.1 link). Then, go to [Pfam scan](http://https://www.ebi.ac.uk/Tools/pfa/pfamscan//) and search the sequence you just copied on the Sequence Search window. In the *Output format* menu, select `Text`. 
...
13. Select the <hmm acc> domain name with the best E-value. Go to [PFAM](http://pfam.xfam.org/) and paste the <hmm acc> domain name in the text box in front of the *jump to* link and press GO. Answer these questions:

```{block, type='rmdquestion'}
### Questions 24 - 25

-  How many architectures of this domain are found?
-  How many sequences are found that contain this domain? 
```
...
-  How many sequences are in the `unknown_sequences.fasta` and the `Phytophthora-ID-ITS-version-1.17.fasta` files? Add the code you used to check the sequences.
...
-  Within the `Phytophthora-ID-ITS-version-1.17.fasta` you can extract the names of each species in the database. What are the species of each of the most likly sequences?
```
...
2. Click on each of the domains. Go to the **Curation & model** link on the left.

3. Download the raw HMM for each domain. 

4. Log into the Cluster. Within the `Lab_4` folder create a `hmm_example` folder.

5. Upload the HMM files and the `oh_this_is_a_mess.fasta` files into the new folder
