...
author: "Johnny, Mathew and Henna"
date: "9/9/2020"
output: html_document
---
# GRADE: 
<h2>
<span style="color: red;">100/100. Excellent job! No comments</span>.
</h2>
...
```
Add you answers here: cd command to access MBB101, mkdir to create Lab_3 folder, cd to open it and wget to download the file. 
...
```
Add you answer here: It is a sequence of DNA because it  contains A, C, G and T. 
...
```
Add you answer here: It is the sequence of amino acids because it has more letters than DNA. 
...
```
Add you answer here: There is one line.
...
```
Add you answer here: There are 541 characters.
...
```
Add you answer here: There are 171, 104, 131, and 134, A, C, G,and T respectively.
...
```
Add you answer here: s/T/U/g S79522.seq > S79522_RNA.seq
...
```
Add you answer here: Open the new file using cat command and counting the number of U. The letter U occured 134 times in this sequence. 
...
```
Add your answer here: There are 11 start codons (AUG), 3 stop codons UAG, 14 UGA and 10 UAA. 
...
```
Add you answer here: There are 157 characters in the AAB21188.seq file. 
...
```
Add you answer here: There are 25 K (lysine) and 6 Y (tyrosine). 
...
```
Add you answer here: Yes, the results of 171 A match with the one at the beginning of the lab. The command -l is only to extract the number of lines.
...
```bash
#Add you answer here: 
aaT=$(grep -o T AAB21188.seq | wc -l)
echo 'Number of T:'$aaT
Number of T:10

aaA=$(grep -o A AAB21188.seq | wc -l)
echo 'Number of A:'$aaA
Number of A:6

aaY=$(grep -o Y AAB21188.seq | wc -l)
echo 'Number of Y:'$aaY
Number of Y:6

aaW=$(grep -o W AAB21188.seq | wc -l)
echo 'Number of W:'$aaW
Number of W:0

aaC=$(grep -o C AAB21188.seq | wc -l)
echo 'Number of C:'$aaC
Number of C:6

aaK=$(grep -o K AAB21188.seq | wc -l)
echo 'Number of K:'$aaK
Number of K:25

aaR=$(grep -o R AAB21188.seq | wc -l)
echo 'Number of R:'$aaR
Number of R:10

aaH=$(grep -o H AAB21188.seq | wc -l)
echo 'Number of H:'$aaH
Number of H:4

aaL=$(grep -o L AAB21188.seq | wc -l)
echo 'Number of L:'$aaL
Number of L:13

aaM=$(grep -o M AAB21188.seq | wc -l)
echo 'Number of M:'$aaM
Number of M:2
...
```
#Add you answer here: We accessed the folder using cd command and checked the sequence files using ls command.
...
```
#Add you answer here: The Fasta file contains the title of the sequence and the sequence itself in a different line. The Genbank file contains a lot of information about the sequence such as taxonomical information, publication data, origin and other data and then the sequence is listed and categorized.
...
```
#Add you answer here: The FASTA files includes RNA, DNA and aminoacid data whereas Genbank only has DNA data.
...
```
#Add you answer here: The 'sequence_1' file is a FASTA file. The title is listed with ">" which confirms it as FASTA file, followed by the sequence.
...
```
#Add you answer here: The 'sequence_2' file is a GenBank file. The file contains different information about the sequence such as locus, source, organism and a lot of more information followed by the sequence which confirms it as a Genbank file. Also, the sequence is divided into categories like gene and CDS. 
...
```
#Add you answer here: There are 10 lines in the 'sequence_1' file. 
...
```
#Add you answer here: The number of sequences that excist in the 'sequence_1' file is half the number of lines. The other way to count the number of sequence is using the command grep -c ">" sequence_1 
...
```
#Add you answer here: There are 5 sequences in the 'sequence_1' file. Yes, this is because one line is for the title and other for the sequence. 
...
```
#Add you answer here: The new sequence was saved in the FASTA file using the command: sed -n '1,2 p' sequence_1 > FASTA_1.fasta. The '>' was used to extract the sequence into the new file.
...
sed -n '1,2 p' sequence_1 > FASTA_1.fasta
sed -n '3,4 p' sequence_1 > FASTA_2.fasta
sed -n '5,6 p' sequence_1 > FASTA_3.fasta
sed -n '7,8 p' sequence_1 > FASTA_4.fasta
sed -n '9,10 p' sequence_1 > FASTA_5.fasta
...
```
#Add you answer here: 
The command used to find T in each sequece grep -o T A_file.fasta | wc -l. For finding other characters, we changed the character and the name of the file. 
```

|Nucleotide| Seq 1| Seq 2| Seq 3| Seq 4| Seq 5|
|---|---|---|---|---|---|
|A|1739| 240| 679| 240| 189|
|C| 1991| 300|841| 284|125|
|G| 2004| 311|887| 297| 137|
|T| 1743|232| 668| 433| 161|

```
2. The length of sequence 1, 2, 3, 4 and 5 is 7478, 1084, 3076, 1255 and 613 respectively.

3. The number of open reading frame in sequence 1, 2, 3, 4 and 5 is 135, 18, 58, 21 and 11 respectively. 
...
```
#Add you answer here: There are 310 lines in 'sequence_2' file. 
...
```
#Add you answer here: 
1. The grep command is used to find lacZ. 
2. The name of the species is Escherichia coli. 
3. The genomic region is the Lactose operon. 
4. There are 4 genes. They are lacI, lacZ, lacY and lacA genes.
...
- mRNA: The mRNA is larger because it includes the repressor. 

- gene: The gene is same as the CDS because there are no introns and exons. 

- CDS: The CDS is the sequence which is going to be transcribed and translated. 
