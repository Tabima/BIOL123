...
# GRADE: 
<h2><span style="color: red;">77/100</span>.
<p>
<span style="color: red;">Please add the names of the members of the group!! (-2 points)</span></h2>
...
```
Add you answers here: wget [web address]
...
```
Add you answer here: It is a DNA sequence.
...
```
Add you answer here: Its a list of amino acids, probably a protein sequence.
...
```
Add you answer here: A-171, C-104, G-131, T-134
...
```
Add you answer here:  cat S79522_RNA.seq and look for the presence of U's. You could also use grep 'U' S79522_RNA.seq and see if it returns with any Us
...
```
Add your answer here: AUG-1, UAG-1, UGA-1, UAA-1

```

<h4><span style="color: red;">All, this is wrong. The answer is 11 start codon and there are 3 UAG, 10 UAA, and 14 UGA (-5 points)</span></h4>
...
```
Add you answer here: 
ARG: CGA-1, CGU-1, CGC-1, CGG-1
HIST: CAU-1, CAC-1

 grep '[Amino Acid]' S79522.seq | wc -l 
```
...
```
Add you answer here: 171 , this result matches the previous command to count the As
```

<h4><span style="color: red;">How are you justifiyng the answer? (-3 pts)</span></h4>
...
```bash
#Add you answer here: nucT=$(grep -o T S79522.seq | wc -l) nucG=$(grep -o G S79522.seq | wc -l)
...
```bash
#Add you answer here: 
[kjoyce@nan Lab_3]$ grep -io 'A' AAB21188.seq | wc -l 
6
[kjoyce@nan Lab_3]$ grep -io 'R' AAB21188.seq | wc -l 
10
[kjoyce@nan Lab_3]$ grep -io 'N' AAB21188.seq | wc -l 
5
[kjoyce@nan Lab_3]$ grep -io 'D' AAB21188.seq | wc -l 
9
[kjoyce@nan Lab_3]$ grep -io 'C' AAB21188.seq | wc -l 
6
[kjoyce@nan Lab_3]$ grep -io 'Q' AAB21188.seq | wc -l 
6
[kjoyce@nan Lab_3]$ grep -io 'E' AAB21188.seq | wc -l 
10
[kjoyce@nan Lab_3]$ grep -io 'G' AAB21188.seq | wc -l 
10
[kjoyce@nan Lab_3]$ grep -io 'I' AAB21188.seq | wc -l 
8
[kjoyce@nan Lab_3]$ grep -io 'L' AAB21188.seq | wc -l 
13
...
```
#Add you answer here: I moved into my mystery_genes and then I used ls to see the files
```
...
```
#Add you answer here: A FASTA file has the header and the gene info after the header. A GenBank file has much more information including the authors, publications, refrence sequences, etc. 
...
```
#Add you answer here: FASTA because it has a small header with the gene info after the header 
...
```
#Add you answer here: GenBank because it doesnt have a simple header and it has a lot of information after the header 
...
```
#Add you answer here: grep '>' sequence_1 | wc
...
```
#Add you answer here: 5, yes it differs because each sequence can have more than one line
...
```
#Add you answer here: sed -n '1,2 p' sequence_1 | grep -v ">" > sequence_1new
...
```
#Add you answer here: [kjoyce@nan mystery_genes]$ sed -n '1,2 p' sequence_1 | grep -v ">" > sequence_1new2
[kjoyce@nan mystery_genes]$ sed -n '3,4 p' sequence_1 | grep -v ">" > sequence_1new3
[kjoyce@nan mystery_genes]$ sed -n '5,6 p' sequence_1 | grep -v ">" > sequence_1new4
[kjoyce@nan mystery_genes]$ sed -n '7,8 p' sequence_1 | grep -v ">" > sequence_1new5
[kjoyce@nan mystery_genes]$ sed -n '9,10 p' sequence_1 | grep -v ">" > sequence_1new6
...
```
#Add you answer here: 
For each file and each letter change accordingly:
1. sed '2,2 p' sequence_1.1| grep -v '>'| grep -o 'A'| wc -l
2. sed '1,1 p' sequence_1new | grep -v '>' | wc -c
3. 270 ORF 
...
1. 130
2. E. coli
3. Genomic DNA from the E. coli 
4. LacA, lacI, lacY, lacZ
...
- mRNA: tells what the mRNA of the gene does
- gene: what the gene name is 
- CDS: contains the actual sequence, the codon start, protein ID, and the gene name
