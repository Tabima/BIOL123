...
I have created a file for us to do a quick and fun exercise today and have uploaded it to a public repository of files called GitHub, at this link: `https://raw.githubusercontent.com/Tabima/BIOL123/refs/heads/main/Lab2/text_file.txt`. 
...
- How many times does the word `Donut` appear? Is it the same number as in question 7 when you checked the number of times that Donut was mentioned? Why?
```
...
1. Remove the `Javier_lines.txt` and the `modified_file.txt` files from the folder. 
...
3. Go back to your `Lab_2/` folder (remember to use the `cd ..` command) and create a symbolic link of your `javier_cat.txt` file into the `Lab_2/` folder using the `ln -s modified/javier_cat.txt .`. 
